...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var annHeight = 11;
   var annStyle = _U.update($Text.defaultStyle,{typeface: _U.list(["Andale Mono"]),height: $Maybe.Just(annHeight)});
   var lcdHeight = 23;
   var lcdStyle = _U.update($Text.defaultStyle,{typeface: _U.list(["HP41"]),height: $Maybe.Just(lcdHeight)});
   var calcHeight = 557;
   var calcWidth = 298;
   var background = A3($Graphics$Element.image,calcWidth,calcHeight,"image/hp41.png");
   var lcdContainer = A4($Graphics$Element.container,
   calcWidth,
   86,
   $Graphics$Element.middle,
   A2($Graphics$Element.width,
   calcWidth,
   A2($Graphics$Element.height,lcdHeight,$Graphics$Element.centered(A2($Text.style,lcdStyle,$Text.fromString("HELLO WORLD2"))))));
   var ann = A4($Graphics$Element.container,
   calcWidth,
   65,
   $Graphics$Element.midBottom,
   A2($Graphics$Element.width,
   calcWidth,
   A2($Graphics$Element.height,annHeight,$Graphics$Element.centered(A2($Text.style,annStyle,$Text.fromString("BAT USER GRAD SHIFT O1234 PRGM ALPHA"))))));
   var calc = A2($Graphics$Element.flow,$Graphics$Element.outward,_U.list([background,lcdContainer,ann]));
   var main = calc;
